...
function AgentOutputCard({ agent, data, model }) {
  ...
  return (
    <Accordion defaultExpanded={false} sx={{ mb: 1 }}>
      <AccordionSummary
        expandIcon={<ExpandMoreIcon sx={{ color: agentMeta.color }} />}
      >
        <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
          <Box sx={{ color: agentMeta.color, display: "flex", fontSize: 20 }}>
            {agentMeta.icon}
          </Box>
          <Typography
            sx={{
          ...
          <Chip
            ...
            size="small"
            sx={{
              bgcolor: alpha(agentMeta.color, 0.1),
              color: agentMeta.color,
              border: `1px solid ${alpha(agentMeta.color, 0.3)}`,
            }}
          />
          {model && (
            <Chip
      ...
      <AccordionDetails>
        <Paper
          sx={{
            bgcolor: "#020912",
            border: `1px solid ${alpha(agentMeta.color, 0.15)}`,
            p: 2,
            borderRadius: 2,
            maxHeight: 320,
            overflow: "auto",
...
function WorkflowTimeline({ result, loading, agentStatus }) {
  return (
    <Stepper
      ...
      activeStep={AGENTS.findIndex(
        (agent) => agentStatus[agent.key] === "running",
      )}
      sx={{
    ...
    >
      {AGENTS.map((agent, idx) => {
        const done = agentStatus[agent.key] === "completed";

        const active = agentStatus[agent.key] === "running";
        return (
          <Step key={agent.key} completed={!!done}>
              ...
              StepIconComponent={() => (
                <Box
                  sx={{
                    ...
                    borderRadius: "50%",
                    bgcolor: done
                      ? alpha(agent.color, 0.15)
                      : active
                        ? alpha(agent.color, 0.08)
                        : "transparent",
                    ...
                    color:
                      done || active ? agent.color : "rgba(255,255,255,0.3)",
                    transition: "all 0.4s ease",
                    boxShadow:
                      done || active
                        ? `0 0 10px ${alpha(agent.color, 0.4)}`
                        : "none",
                  }}
                >
                  {done ? (
            ...
            >
              <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                <Box
                  sx={{
                    color: done || active ? agent.color : "text.secondary",
                    display: "flex",
                  }}
                >
                  {agent.icon}
        ...
        <Accordion key={i} sx={{ mb: 1 }}>
          ...
          <AccordionDetails>
            ...
            >
              ...
              >
                {typeof run === "string" ? run : JSON.stringify(run, null, 2)}
              </pre>
            </Paper>
          </AccordionDetails>
        </Accordion>
...
export default function App() {
  ...
  const updateAgentStatus = (logLines) => {
    ...
    setAgentStatus({
      research: joined.includes("Research Agent Completed")
        ? "completed"
        : joined.includes("Research Agent Started")
          ? "running"
          : "pending",
      strategy: joined.includes("Strategy Agent Executed")
        ? "completed"
        : joined.includes("Strategy Agent Started")
          ? "running"
          : "pending",
      critic: joined.includes("Critic Agent Executed")
        ? "completed"
        : joined.includes("Critic Agent Started")
          ? "running"
          : "pending",
      planner: joined.includes("Planner Agent Executed")
        ? "completed"
        : joined.includes("Planner Agent Started")
          ? "running"
          : "pending",
      qa: joined.includes("QA Agent Executed")
        ? "completed"
        : joined.includes("QA Agent Started")
          ? "running"
          : "pending",
    });
  };
    ...
    try {
      ...
      const interval = setInterval(async () => {
        try {
          ...
          const logsResponse = await axios.get("http://127.0.0.1:8000/logs");
          ...
          logs.forEach((log) => {
            if (log.includes("Research Agent Completed")) {
              updated.research = "completed";
              updated.strategy = "running";
            }

            if (log.includes("Strategy Agent Executed")) {
              updated.strategy = "completed";
              updated.critic = "running";
            }

            if (log.includes("Critic Agent Executed")) {
              updated.critic = "completed";
              updated.planner = "running";
            }

            if (log.includes("Planner Agent Executed")) {
              updated.planner = "completed";
              updated.qa = "running";
            }

            if (log.includes("QA Agent Executed")) {
...
${cleanText(result.strategy?.slice(0, 900))}

-----------------------------------

KEY RISKS & CRITICAL INSIGHTS

${cleanText(result.critic?.slice(0, 700))}

-----------------------------------

EXECUTION ROADMAP

${cleanText(result.planner?.slice(0, 900))}

-----------------------------------

FINAL VALIDATION

${cleanText(result.qa?.slice(0, 700))}
`;
  };
  return (
      ...
      >
        ...
        <Box sx={{ p: 2 }}>
          ...
          {[
            ...
            >
              {icon}
              <Typography
                sx={{ fontSize: "0.82rem", fontWeight: active ? 700 : 400 }}
              >
                {label}
              </Typography>
              {active && (
        ...
        <Box sx={{ p: 2 }}>
          ...
          {AGENTS.map((agent) => (
            ...
            >
              <GlowDot
                color={result ? agent.color : loading ? "#ffab40" : "#334155"}
                pulse={loading}
              />
              <Box
                sx={{
                  color: result ? agent.color : "text.secondary",
                  display: "flex",
                  fontSize: 16,
                }}
              >
        ...
        <Box
          sx={{ mt: "auto", p: 2, borderTop: "1px solid rgba(0,229,255,0.1)" }}
        >
          <Box
            sx={{
              display: "flex",
            ...
            {loading && (
              <Chip
                icon={
                  <CircularProgress
                    size={12}
                    sx={{ color: "warning.main !important" }}
                  />
                }
                label="WORKFLOW RUNNING"
                size="small"
                sx={{
                  bgcolor: alpha("#ffab40", 0.1),
                  color: "warning.main",
        ...
        <Box sx={{ p: 3 }}>
          <Grid container spacing={2} sx={{ mb: 3 }}>
            {[
              {
                label: "Active Agents",
                value: result ? "5/5" : loading ? "1/5" : "0/5",
                icon: <SmartToyIcon />,
                color: "#00e5ff",
              },
              {
              ...
              {
                label: "Status",
                value: result ? "Done" : loading ? "Running" : "Idle",
                icon: <HubIcon />,
              ...
              <Card sx={{ mb: 3 }}>
                <CardContent sx={{ p: 3 }}>
                  ...
                  >
                    ...
                    <Box sx={{ ml: "auto" }}>
                      <Chip
                        label={`${filledFields}/${FORM_FIELDS.length} FIELDS`}
                        size="small"
                        sx={{
                  ...
                  >
                    <Button
                      ...
                      sx={{
                        background: loading
                          ? undefined
                          : "linear-gradient(135deg, #00e5ff, #0091a8)",
                        color: "#050d1a",
                        fontWeight: 900,
                        fontSize: "0.75rem",
                        "&:hover": {
                          background:
                            "linear-gradient(135deg, #6effff, #00e5ff)",
                        },
                        "&.Mui-disabled": {
                          background: alpha("#00e5ff", 0.1),
                          color: alpha("#00e5ff", 0.4),
                        },
                      }}
                    >
                    ...
                    >
                      <Box
                        sx={{
                          p: 2,
                          background:
                            "linear-gradient(90deg,#00e5ff22,#7c4dff22)",
                        }}
                      >
                        <Typography
                          variant="h5"
                        ...
                        >
                          Combined executive-level output generated by the
...
${cleanText(result.strategy?.slice(0, 1200))}

--------------------------------------------------

KEY RISKS & CRITICAL INSIGHTS

${cleanText(result.critic?.slice(0, 900))}

--------------------------------------------------

EXECUTION ROADMAP

${cleanText(result.planner?.slice(0, 1200))}

--------------------------------------------------

FINAL VALIDATION

${cleanText(result.qa?.slice(0, 900))}
`}
                          </Typography>
                        </Paper>
                      </CardContent>
            ...
            <Grid size={{ xs: 12, md: 5 }}>
              <Card sx={{ mb: 3 }}>
                <CardContent sx={{ p: 3 }}>
                  ...
                  <WorkflowGraph agentStatus={agentStatus} />
